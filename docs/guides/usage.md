# Usage Guide
This guide explores usage and configuration scenarios for Agent ZERO V 2.2. You can consider this as a reference post-installation guide.

![Utility Message with Solutions](../res/usage/first-task.png)

## Basic Operations
Agent ZERO V 2.2 provides several basic operations through its interface:

### Restart Framework
The Restart button allows you to quickly restart the Agent ZERO V 2.2 container without using the terminal:
...
### Action Buttons
Located beneath the chat input box, Agent ZERO V 2.2 provides a set of action buttons for enhanced control and visibility:
...
### File Browser: Manage files in the Agent ZERO V 2.2 environment
...
### File Attachments
Agent ZERO V 2.2 supports direct file attachments in the chat interface for seamless file operations:
...
## Tool Usage
Agent ZERO V 2.2's power comes from its ability to use [tools](../developer/architecture.md#tools). Here's how to leverage them effectively:

- **Understand Tools:** Agent ZERO V 2.2 includes default tools like knowledge (powered by SearXNG), code execution, and communication. Understand the capabilities of these tools and how to invoke them.
...
Agent ZERO V 2.2 instances can communicate with each other using the A2A protocol. This enables:
...
To enable A2A connectivity, go to **Settings → MCP/A2A → Nuvho A2A Server** and toggle the server on. You'll receive a connection URL that other Agent ZERO V 2.2 instances can use to communicate with your agent.

See [A2A Setup](a2a-setup.md) for detailed configuration and use cases.

## Example of Tools Usage: Web Search and Code Execution
Let's say you want Agent ZERO V 2.2 to perform some financial analysis tasks. Here's a possible prompt:
...
> [!NOTE]
> The first run of `code_execution_tool` may take a while as it downloads and builds the Agent ZERO V 2.2 Docker image. Subsequent runs will be faster.

This example demonstrates how to combine multiple tools to achieve an analysis task. By mastering prompt engineering and tool usage, you can unlock the full potential of Agent ZERO V 2.2 to solve complex problems.

## Multi-Agent Cooperation
One of Agent ZERO V 2.2's unique features is multi-agent cooperation.
...
* **Hierarchy:** Agent ZERO V 2.2 uses a [hierarchical structure](../developer/architecture.md#agent-hierarchy-and-communication), with superior agents delegating tasks to subordinates.  This allows for structured problem-solving and efficient resource allocation.

![](../res/usage/multi-agent.png)

## Projects
Projects are isolated workspaces that provide dedicated context, instructions, memory, and secrets for specific tasks or clients. They are one of Agent ZERO V 2.2's most powerful organizational features, preventing context bleed and enabling focused, specialized agent behavior.
...
3. Agent ZERO V 2.2 clones the repository and sets up the project structure
...
> [!NOTE]
> If the cloned repository already contains a `.a0proj/` configuration folder, Agent ZERO V 2.2 merges the existing configuration with your specified preferences.
...
## Tasks & Scheduling
Tasks enable Agent ZERO V 2.2 to run automated or scheduled work in isolated contexts. They're perfect for recurring workflows, batch processing, or time-delayed operations that don't require immediate attention.

### What Are Tasks?

Tasks are autonomous work units that Agent ZERO V 2.2 executes in dedicated or shared chat contexts. Each task includes:
...
Agent ZERO V 2.2 supports three types of tasks:
...
Agent ZERO V 2.2 includes a secure tunneling feature that allows you to expose your local instance to the internet. This makes it possible to access your Agent ZERO V 2.2 instance from anywhere or share it with others without complex network configuration.

### How Tunneling Works

Agent ZERO V 2.2 uses the [Flaredantic](https://pypi.org/project/flaredantic/) library to create secure tunnels. These tunnels:
...
6. The tunnel URL remains active until you stop the tunnel or close Agent ZERO V 2.2

### Security Considerations

When sharing your Agent ZERO V 2.2 instance via a tunnel:

- Anyone with the URL can access your Agent ZERO V 2.2 instance
- No additional authentication is added beyond what your Agent ZERO V 2.2 instance already has
- **Always set up authentication before creating a tunnel** (see below)
- The tunnel exposes only your Agent ZERO V 2.2 instance, not your entire system

> [!IMPORTANT]
> When attempting to create a tunnel without authentication configured, Agent ZERO V 2.2 will display a security warning.

### Adding Authentication for Tunnels

To secure your tunneled Agent ZERO V 2.2 instance, configure authentication in Settings:

1. Open **Settings** in the Agent ZERO V 2.2 UI
...
This will require users to enter these credentials when accessing your tunneled Agent ZERO V 2.2 instance.
...
3. Restart Agent ZERO V 2.2
4. Check the console logs for any error messages

> [!TIP]
> Combine tunneling with authentication for secure remote access to your Agent ZERO V 2.2 instance from any device, including mobile phones and tablets.

## Voice Interface
Agent ZERO V 2.2 provides both Text-to-Speech (TTS) and Speech-to-Text (STT) capabilities for natural voice interaction:
...
### File Browser
Agent ZERO V 2.2 provides a powerful file browser interface for managing your workspace:
...
> [!NOTE]
> The file browser lets you navigate the Agent ZERO V 2.2 filesystem. For file-based work, keep your working files in `/a0/usr` (or inside a Project workspace).
>
- **File Operations**:
...
> [!TIP]
> The File Browser integrates seamlessly with Agent ZERO V 2.2's capabilities. You can reference files directly in your conversations, and the agent can help you manage, modify, and organize your files.

## Memory Management
Agent ZERO V 2.2 includes a sophisticated memory management system that stores and retrieves information from conversations, knowledge sources, and learning experiences. The Memory Dashboard provides a powerful interface to view, search, filter, edit, and delete memory entries stored in the vector database.
...
## Backup & Restore
Agent ZERO V 2.2 provides a comprehensive backup and restore system to protect your data and configurations. This feature helps you safeguard your work and migrate Agent ZERO V 2.2 setups between different systems.
...
#### What Gets Backed Up
By default, Agent ZERO V 2.2 backs up your most important data:
...
The restore process allows you to recover your Agent ZERO V 2.2 setup from a previous backup:
...
* **Before System Updates**: Backup before updating Agent ZERO V 2.2 or system components
...
* **External Storage**: Keep backup files in a safe location outside Agent ZERO V 2.2
...
#### System Migration
Moving Agent ZERO V 2.2 to a new server or computer:
1. Create a complete backup on the original system
2. Install Agent ZERO V 2.2 on the new system
...
#### Team Collaboration
Sharing Agent ZERO V 2.2 configurations:
...
> [!TIP]
> The backup system is designed to work across different operating systems and Agent ZERO V 2.2 installations. Your backups from a Windows system will work on Linux, and vice versa.
