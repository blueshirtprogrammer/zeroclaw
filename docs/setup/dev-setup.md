# Development manual for Agent ZERO V 2.2
This guide will show you how to setup a local development environment for Agent ZERO V 2.2 in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - Agent ZERO V 2.2 runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
- Agent ZERO V 2.2 is available on GitHub [github.com/agent0ai/agent-zero-x](https://github.com/agent0ai/agent-zero-x).
- You can download the files using a browser and extract or run `git clone https://github.com/agent0ai/agent-zero-x` in your desired directory.

> [!NOTE]
> In my case, I used `cd ~/Desktop` and `git clone https://github.com/agent0ai/agent-zero-x`, so my project folder is `~/Desktop/agent-zero-x`.
...
- Agent ZERO V 2.2 comes with `.vscode` folder that contains basic setup, recommended extensions, and debugger profiles. These will help us a lot.

1. Open your IDE and open the project folder using `File > Open Folder` and select your folder, in my case `~/Desktop/agent-zero-x`.
...
## Step 4: Run Agent ZERO V 2.2 in the IDE
Great work! Now you should be able to run Agent ZERO V 2.2 from your IDE including real-time debugging.
...
After inserting my API key in settings, my Agent ZERO V 2.2 instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of Agent ZERO V 2.2 in Docker
...
1. Pull the docker image `agent0ai/agent-zero-x` from Docker Hub and run it with a web port (`80`) mapped and SSH port (`22`) mapped.
...
This is how it looks in my example: port `80` is mapped to `8880` on the host and `22` to `8822`, `/a0` folder mapped to `/Users/frdel/Desktop/agent-zero-x`:
...
Agent ZERO V 2.2 runs code inside the container by default. If you are running the framework locally in your IDE but want tools (like code execution) to run in Docker, configure RFC in **Settings -> Development** and point it to a running Agent ZERO V 2.2 container. This routes execution through SSH/RFC to the container while keeping the UI and agent loop on your host.


# Congratulations!

You have successfully set up a complete Agent ZERO V 2.2 development environment! You now have:
...
- The ability to develop, debug, and test Agent ZERO V 2.2 features seamlessly

You're now ready to contribute to Agent ZERO V 2.2, create custom extensions, or modify the framework to suit your needs. Happy coding!
...
- Navigate to your project root in the terminal and run `docker build -f DockerfileLocal -t agent-zero-x-local --build-arg CACHE_DATE=$(date +%Y-%m-%d:%H:%M:%S) .`
